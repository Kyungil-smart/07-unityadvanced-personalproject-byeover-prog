...
namespace _Game.Scripts.Rhythm
{
    ...
    public sealed class StagePatternRunner : MonoBehaviour
    {
        [Header("참조")]
        [Tooltip("리듬 시간 기준(필수)")]
        ...
        private IEnumerator Stage1Routine()//이 방법밖에 없는걸까??
        {
            yield return RhythmWait.After(conductor, 0.65);
            yield return BossPulse();

            yield return RhythmWait.After(conductor, 1.30);
            yield return BossPulse();

            yield return RhythmWait.After(conductor, 0.75);
            yield return BossPulse();
        }

        private IEnumerator BossPulse()//yiled~~~
        {
            if (bossTransform == null) yield break;

            bossTransform.localPosition = _bossBaseLocalPos + new Vector3(0f, bossPulseHeight, 0f);
            yield return RhythmWait.After(conductor, bossPulseHold);
