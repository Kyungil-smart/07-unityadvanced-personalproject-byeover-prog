...
namespace _Game.Scripts.Rhythm
{
    ...
    public sealed class RhythmFeedbackHub2D : MonoBehaviour
    {
        [Header("참조")]
        [Tooltip("판정 시스템")]
        ...
        [Tooltip("보스 피격 반응(선택)")]
        [SerializeField] private BossHitReact2D bossHitReact;

        [Tooltip("히트바 이펙트(선택)")]
        [SerializeField] private HitBarSpawner2D hitBarSpawner;
        ...
        private void Awake()
        {
            if (judgeSystem == null) judgeSystem = FindFirstObjectByType<JudgeSystem2D>();
            if (bossHitReact == null) bossHitReact = FindFirstObjectByType<BossHitReact2D>();
            if (hitBarSpawner == null) hitBarSpawner = FindFirstObjectByType<HitBarSpawner2D>();
        }
